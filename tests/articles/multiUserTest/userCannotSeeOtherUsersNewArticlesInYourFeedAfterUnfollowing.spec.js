...
test("User doesn't see other user's articles in "
  + "Your Feed after unfollowing their profile", async ({
    page1,
    page2,
    user1,
    user2,
  ...
  await viewArticlePage2.open(articleWithoutTags.url);
  await viewArticlePage2.clickOnUnfollowBtn();
  await viewArticlePage2.assertFollowBtnIsVisible();

  await homePage.open();
  await homePage.clickYourFeedTabLink();
  await homePage.assertYourFeedTabIsVisible();
  await homePage.assertArticleInFeedTabIsNotVisible(articleWithoutTags.title);
  await homePage.assertArticleInFeedTabIsNotVisible(articleWithOneTag.title);
  await homePage.assertArticleAuthorNameIsNotVisible(user1.username);
  await homePage.assertUsernameIsVisible(user2.username);
});
